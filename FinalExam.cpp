...
the usage
// of each resource(chopstick). Initially, the semaphore value for each
chopstick is 1.
...
// THAT IS WHY use need to use &, because the semaphore need to be
modifie
sem_wait(&m);
...
printf("Philosopher %d picked up Chopstick %d, can NOT eat with one
chopstick.\n", i, right);
// wait to use the left chopstick
sem_wait(&chopsticks[left]);
printf("Philosopher %d picked up Chopstick %d, now is eating with two
chopsticks.\n", i, left);
delay(60000);
...
*(*start)(void *), void *arg);
//Parameters--> 1st: thread id; 2nd: NULL; 3rd: a proper prededined function
4th: what you want to pass to the new thread
for (i=0; i<N; i++) {
